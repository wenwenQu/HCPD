...
namespace graphlab {
  ...
  public:
...
#endif
    {
      ...
      topoX_rpc.barrier();
        count = (size_t *) malloc((topoX_rpc.numprocs() / ceng)*sizeof (size_t));
    } // end of constructor
...
#else
		if (sh != th)
			topoX_edge_exchange.send(sh, record);                  //��һ���߷���source��target���ڵ������ڵ��£����λ��ͬһ�ڵ㣬ֻ����һ��
      ...
      /**************************************************************************/
      {
        ...
        if (standalone) { /* fast pass for standalone */
          ...
          while(topoX_edge_exchange.recv(proc, edge_buffer)) {
				...
				foreach(const edge_buffer_record& rec, edge_buffer){         //��Ŀ�ĵ�洢�ñߣ������ݱ߹���iht��oht
 					if (rec.target%nprocs == l_procid){
						topoX_edges.push_back(rec);
					...
					fht[rec.source] = -1;                                 //fht���ã�mapping proc hash table
					fht[rec.target] = -1;
					flaght[rec.target] = 0;
					flaght[rec.source] = 0;
			  ...
			  if (!p_queue.empty()){
				  ...
				  if (flaght[iter->first] == 0 && dht[iter->first]<=threshold){
					  ...
					  if (fht[iter->first] == -1){
						  ...
						  fht[iter->first] = min_proc;                  //��һ�θ���fht����ʱֻ���±���fht
					  }
					  else {
