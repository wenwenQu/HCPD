...
namespace graphlab {
  ...
  public:
    ...
    /// detail vertex record for the second pass coordination.
      ...
      size_t* count;         //记录各节点现有的分配到的边的数目，如何分布式？
...
#else
          if (sh != th)
              matrix_edge_exchange.send(sh, record);                  //��һ���߷���source��target���ڵ������ڵ��£����λ��ͬһ�ڵ㣬ֻ����һ��
              ...
              if (standalone) { /* fast pass for standalone */
                  ...
                  while(matrix_edge_exchange.recv(proc, edge_buffer)) {
                      ...
                      foreach(const edge_buffer_record& rec, edge_buffer){         //按目的点存储该边，并根据边构建iht和oht
                          ...
                          fht[rec.source] = -1;                                 //fht作用：mapping proc hash table
                          fht[rec.target] = -1;
                          flaght[rec.target] = 0;
                          flaght[rec.source] = 0;
                          ...
                          if (mht.find(rec.target) == mht.end()) {
                              ...
                              for (size_t j = 0; j < iht[rec.target].size(); j++){
                                  if (fht[iht[rec.target][j]] == -1 && in_degree_set[iht[rec.target][j]] <=threshold){
                                      ...
                                      add_queue(iht[rec.target][j], owning_proc, rec.target, countht[rec.target]);
                                  }
                              }
                              for (size_t j = 0; j < oht[rec.target].size(); j++){
                                  if (fht[oht[rec.target][j]] == -1 && in_degree_set[oht[rec.target][j]] <=threshold){
                                      ...
                                      add_queue(oht[rec.target][j], owning_proc, rec.target, countht[rec.target]);
                                  }
                              }
                          }
...
*/
                	  ...
                	  if (in_degree_set[iter->first] > threshold){
                		  ...
                		  for (size_t j = 0; j < iht[iter->first].size(); j++){
                			  if(in_degree_set[iht[iter->first][j]] > threshold){
                                  int row, col;
                                  col = (iter->first) % matrixSize;
                                  row = (iht[iter->first][j]) % matrixSize;
                                  if (col == row) col = rand() % matrixSize;
                		  ...
                		  if(owning_proc == -1) continue;
                		  else owning_proc = (procid_t)owning_proc;
                          for (size_t j = 0; j < iht[iter->first].size(); j++){
                              if (fht[iht[iter->first][j]] == -1 && in_degree_set[iht[iter->first][j]] <=threshold){
                                  fht[iht[iter->first][j]] = owning_proc;
                                  add_queue(iht[iter->first][j], owning_proc, iter->first, countht[iter->first]);
                              }
                          }
                          for (size_t j = 0; j < oht[iter->first].size(); j++){
                              if (fht[oht[iter->first][j]] == -1 && in_degree_set[oht[iter->first][j]] <=threshold){
                                  fht[oht[iter->first][j]] = owning_proc;
                                  add_queue(oht[iter->first][j], owning_proc, iter->first, countht[iter->first]);
                              }
                          }

                          for (procid_t p = 0; p < nprocs; ++p) {
                             if (p != l_procid)
                                 mht_exchange.send(p, master_pair_type(iter->first, owning_proc));
                             else
                                 mht[iter->first] = owning_proc; //@wenwen: assigned based matrix or -1
                          }
                	  }
                  }
                  ...
                  for (size_t i = 0; i < matrix_edges.size(); i++){
                      ...
                      if (dht[rec.target] > threshold){
                          if (mht.find(rec.target) != mht.end()){ //have high-degree neighbor
                              int row, col;
                              col = rec.source % matrixSize;
                              row = rec.target % matrixSize;
                              if (col == row) col = rand() % matrixSize;
                              ...
                              owning_proc = rec.target%cnodes; // no high-degree neighbor
                          }
                          if (owning_proc != l_procid){
