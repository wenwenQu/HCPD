 ...
 *
 *
 * 2013.11  implement matrix partitioning with heuristic (ginger)
 *
 */


#ifndef GRAPHLAB_DISTRIBUTED_MATRIX_GINGER_INGRESS_HPP
#define GRAPHLAB_DISTRIBUTED_MATRIX_GINGER_INGRESS_HPP
...
namespace graphlab {
  ...
  /**
   * \brief Ingress object assigning edges using a matrix method.
   *        That is, for high degree edges,
   *        for low degree edges, hashing from its target vertex.
   */
  template<typename VertexData, typename EdgeData>
  class distributed_matrix_ginger_ingress :
  ...
  public:
    ...
    dc_dist_object<distributed_matrix_ginger_ingress> matrix_rpc;
    ...
    master_hash_table_type mht,mht_high;
  ...
  public:
    ...
    distributed_matrix_ginger_ingress(distributed_control& dc, graph_type& graph,
        ...
        mht_exchange(dc), in_degree_exchange(dc), proc_balance(dc.numprocs()),
      ...
      standalone = matrix_rpc.numprocs() == 1;
      matrix_rpc.barrier();
    ...
    ~distributed_matrix_ginger_ingress() { }
    ...
    void finalize() {
      ...
      /*                                                                        */
      /**************************************************************************/
      if (!standalone) assign_matrix_edges();
      ...
      if (standalone) { /* fast pass for standalone */
        ...
        matrix_edges.reserve(nedges + high_edge_exchange.size());
        ...
        // re-send edges of high-degree vertices by matrix_edge_exchange
        proc = -1;
        procid_t owning_proc = 0;
        while(high_edge_exchange.recv(proc, edge_buffer)) {
          foreach(const edge_buffer_record& rec, edge_buffer) {
        	  if(in_degree_set.find(rec.source)==in_degree_set.end()){
        		  owning_proc = graph_hash::hash_vertex(rec.source) % nprocs;;
        	  }
        	  else if(mht.find(rec.source)!=mht.end()){
        		  owning_proc = mht[rec.source];
        	  }
        	  else{
        		  int row,col;
    				...
    				owning_proc = myMatrix[row][col];
      				...
      				if (mht_high.find(rec.target) == mht_high.end()){
      				  // update mht and nedges_incr
      				  for (procid_t p = 0; p < nprocs; ++p) {
      					if (p != l_procid)
      					  mht_exchange.send(p, master_pair_type(rec.target, owning_proc));
      					else
      					  mht_high[rec.target] = owning_proc;
      				  }
      				}
            if (owning_proc == l_procid) {
              ...
              matrix_edge_exchange.send(owning_proc, rec);
            }
          }
        }
        ...
        while(mht_exchange.recv(proc, mht_buffer)){
        	foreach(const master_pair_type& pair, mht_buffer){
        		mht_high[pair.first] = pair.second;
        	}
        }
        mht_exchange.clear();
...
#ifdef TUNING
        logstream(LOG_INFO) << "receive #edges=" << matrix_edge_exchange.size()
                            << std::endl;
#endif
        proc = -1;
        while(matrix_edge_exchange.recv(proc, edge_buffer)) {
    ...
    void modified_base_finalize(size_t nedges) {
      ...
      // Setup the map containing all the vertices being negotiated by this machine
      {
        if (standalone) {
          ...
          procid_t proc = -1;
          while(matrix_vertex_exchange.recv(proc, vertex_buffer)) {
            foreach(const vertex_buffer_record& rec, vertex_buffer) {
              ...
              if (distributed_matrix_ginger_ingress::vertex_combine_strategy
                  && lvid < graph.num_local_vertices()) {
                distributed_matrix_ginger_ingress::vertex_combine_strategy(
      ...
      /**************************************************************************/
      {
        ...
        foreach(const vid2lvid_pair_type& pair, vid2lvid_buffer) {
          ...
          if (standalone) {
            ...
            else if(mht_high.find(pair.first) != mht_high.end())
            	vrec.owner = mht_high[pair.first];
            else
...
#pragma omp parallel
      ...
      // TODO:  optimization for standalone
      {
        ...
        graphlab::graph_gather_apply<graph_type, vertex_negotiator_record>
            vrecord_sync_gas(graph,
                             boost::bind(&distributed_matrix_ginger_ingress::finalize_gather, this, _1, _2),
                             boost::bind(&distributed_matrix_ginger_ingress::finalize_apply, this, _1, _2, _3));
  ...
  }; // end of distributed_matrix_ginger_ingress
