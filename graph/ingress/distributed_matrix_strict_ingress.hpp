...
#ifndef GRAPHLAB_DISTRIBUTED_MATRIX_STRICT_INGRESS_HPP
#define GRAPHLAB_DISTRIBUTED_MATRIX_STRICT_INGRESS_HPP
...
namespace graphlab {
   ...
   */
  template<typename VertexData, typename EdgeData>
  class distributed_matrix_strict_ingress :
  ...
  public:
    ...
    dc_dist_object<distributed_matrix_strict_ingress> matrix_rpc;
  ...
  public:
    ...
    distributed_matrix_strict_ingress(distributed_control& dc,
    ...
    ~distributed_matrix_strict_ingress() { }
    ...
    void add_vertex(vertex_id_type vid, const VertexData& vdata) {
      ...
      const procid_t owning_proc = standalone ? 0 :
        graph_hash::hash_vertex(vid) % matrix_rpc.numprocs();
#ifdef _OPENMP
      ...
      /**************************************************************************/
      {
        ...
        if (standalone) { /* fast pass for standalone */
          ...
          for (size_t i = 0; i < matrix_edges.size(); i++) {
            ...
            if (in_degree_set[rec.target] > threshold && in_degree_set[rec.source] > threshold) {
            	int row,col;
            ...
            else if (in_degree_set[rec.target] > threshold) {
        ...
        if (matrix_vertex_exchange.size() > 0) {
          ...
          while(matrix_vertex_exchange.recv(sending_proc, vertex_buffer)) {
            foreach(const vertex_buffer_record& rec, vertex_buffer) {
              ...
              if (distributed_matrix_strict_ingress::vertex_combine_strategy
                && lvid < graph.num_local_vertices()) {
                distributed_matrix_strict_ingress::vertex_combine_strategy(
      ...
      // TODO:  optimization for standalone
      {
        ...
        graphlab::graph_gather_apply<graph_type, vertex_negotiator_record>
            vrecord_sync_gas(graph,
                             boost::bind(&distributed_matrix_strict_ingress::finalize_gather, this, _1, _2),
                             boost::bind(&distributed_matrix_strict_ingress::finalize_apply, this, _1, _2, _3));
